...
if [ -e /var/db/pkg.real ]
then
	echo "Fixing the packages database..."
...
then
	...
	for PKGS in $PKGSX
	do
	if [ -e compile.list.virtuals/$PKGS.virt ]
	then
		PKGS2="$PKGS"
	...
	if [ "$PKGS" = "gnomex" ] || [ "$PKGS" = "kde-metax" ] || [ "$PKGS" = "lxde-metax" ] || [ "$PKGS" = "xfce4-metax" ] || [ "$PKGS" = "portagex" ] || [ "$PKGS" = "xorg-x11x" ] || [ "$PKGS" = "xorg-serverx" ]
	then
		echo " deep setting"
	...
	if [ -e scripts/compile.list.$PKGNAME ]
	then
		echo "Exist $PKGNAME!"
	else
		mv /var/db/pkg /var/db/pkg.real
		mv /var/db/pkg.kit /var/db/pkg
		if [ "$PKGS" = "gnomex" ] || [ "$PKGS" = "kde-metax" ] || [ "$PKGS" = "lxde-metax" ] || [ "$PKGS" = "system" ] || [ "$PKGS" = "xfce4-metax" ] || [ "$PKGS" = "portagex" ] || [ "$PKGS" = "xorg-x11x" ] || [ "$PKGS" = "xorg-serverx" ]
		then
			echo " deep setting"
			#ACCEPT_KEYWORDS="~x86" USE="$USEX" emerge --backtrack=30 --newuse --update --deep -pv $PKGS &> compile.list/compile.list
			ACCEPT_KEYWORDS="~x86" USE="$USEX" emerge --deep -pv $PKGS &> compile.list/compile.list
