#!/bin/bash
#####################################
# Compilation of packages for i686	#
...
do
	...
	then
		...
		if [ -e packages/$ARQ/$PKGX2.tbz2 ]
		then
			sleep 0
		else
			echo "Looking at $CATE the package $PKGX2 (1)"
			EXISTE=`find /var/db/pkg/$CATE/ -type f -name '*.ebuild' -print | sed -e "s/.ebuild//" | grep "$PKGX2" | cut -d "/" -f 7`
			if [ "$EXISTE" != "$PKGX2" ]
			then
				#wget packages.ututo.org/i686/$PKGX2.tbz2
				if [ -e $PKGX2.tbz2 ]
				then
		...
		then
			...
			echo "Looking at $CATE the package $PKGX2 (2)"
			EXISTE=`find /var/db/pkg/$CATE/ -type f -name '*.ebuild' -print | sed -e "s/.ebuild//" | grep "$PKGX2" | cut -d "/" -f 7`
			if [ "$EXISTE" != "$PKGX2" ]
			then
				clear
				echo "------------------------------------------------------------------"
				echo "Package: $CATE / $PKGX ($EXISTE != $PKGX2)"
				echo "USE: $USEMAKE $USEX"
				echo "------------------------------------------------------------------"
				echo " "
				cp makes.conf.fast/make.conf.$ARQ /etc/make.conf
				USE="$USEMAKE $USEX" emerge --nodeps $PARM =$PKGX
				if [ -e /usr/portage/packages/All/$PKGX2.tbz2 ]
				then
					mkdir packages/$ARQ
		...
		echo "Compiled in the system... $PKG"
    fi
